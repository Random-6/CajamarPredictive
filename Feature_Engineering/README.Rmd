En este apartado se explicarán qué variables se introducirán en los modelos predictivos. 

Para preparar el dataset de train, se ha decidio utilizar el dataset sin los últimas compras, ya que estas se utilizarán para validar nuestro modelo. 
...
data.train.without.last <- as.data.table(read_csv("~/Documents/Random6/CajamarPredictive/Feature_Engineering/Data/data_train_without_last.csv"))
...
Primero de todo, tal y como se vio en el análisis exploratorio de los datos, hay mucha información temporal que nos puede ser útil. Así pues, podemos utilizar las variables creadas en el apartado anterior, como *mes* y *año*, *tiempo desde primera compra*, *tiempo desde última compra* y *tiempo medio entre compras*. Otro concepto interesante es saber cuantos productos ha comprado cada customer por mes, ya que así se intenta modelizar el hecho que haya epocas en que se compran mas unos productos que otros, y que también hay usuarios que tienden a compran más en determinados meses. 

A parte de las variables temporales, tambien se puede introducir información de los productos comprados anteriormente, ya que se ha visto que hay productos que se tienden a comprar juntos o de manera consecutiva, así que tener en cuenta los productos comprados previamente es también importante. 
...
```
Como se puede observar, el clustering de productos según el número de productos por mes de compra no genera resultados muy aprovechables, ya que separa los productos en un cluster muy grande y otros de pequeños. Esta información no nos es muy útiles para añadir información al modelo. 
...
- Producto X comprado SI o NO (94 variables)

De esta manera se han caracterizado los customers segun sus características demograficas y sus características y historial de compras. 
