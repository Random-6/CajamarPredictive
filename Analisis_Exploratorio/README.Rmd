...
## Variables de tiempo
temps_var_per_customer <- data.train.without.last[order(date),.(
...
data.train.without.last[,date := as.Date(date)]
data.train.without.last  <- data.train.without.last[order(date)]
#tiempo entre compras
lag_entre_compres <- data.train.without.last[, .(mean(as.numeric(diff.POSIXt(date)))), by = ID_Customer] 
...
setkey(data.train.without.last, ID_Customer)
data.train.without.last <- data.train.without.last[temps_var_per_customer, on = 'ID_Customer']
```
...
data.train[,Cod_Prod := as.factor(Cod_Prod)]
data.train.without.last[,Cod_Prod := as.factor(Cod_Prod)]
ggplot(data.train.without.last[ID_Customer %in% temps_var_per_customer[is.na(lag_entre_compres), ID_Customer], .N,by = Cod_Prod], aes(x = Cod_Prod, y = N)) + geom_bar(stat = 'identity') + theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
ggplot(data.train.without.last[ID_Customer %in% temps_var_per_customer[total_productes_comprats > 10, ID_Customer],.N,by = Cod_Prod][order(-N)], aes(x = Cod_Prod, y = N)) + geom_bar(stat = 'identity') + theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
data.train.without.last[lag_entre_compres == 0, uniqueN(ID_Customer)]
data.train.without.last[lag_entre_compres == 0, .N]
data.train.without.last[lag_entre_compres == 0, .N, by = ID_Customer][,table(N)]
groups_of_products <- data.train.without.last[lag_entre_compres == 0,paste0(Cod_Prod,collapse = '-'), by = ID_Customer]
...
compres.per.customer.per.mes <- data.train.without.last[,.N,by = c('ID_Customer','month')]
